...
def drawRes():
	global lu_types, cmap, norm
	lu_types = [ 'others', 'urban', 'A', 'B', 'D','background']
	colors_list = [ 'grey', 'black', 'red','green', 'yellow','white']
	cmap = colors.ListedColormap(colors_list)
	...
	cbar=fig.colorbar(cax,ticks=np.arange(6))
	loc=np.arange(6)+0.5 	
	cbar.set_ticks(loc)
	cbar.set_ticklabels(lu_types)
	plt.text(250, 1150, 'merit:'+ str(merit_fig))
	#fig.show()
	res=directory + '/Res_T' + str(T) + '_' + str(a) + '_' + str(b) + '_' + str(radius) + '_' + str(nb_type_index) + '.png' 
	fig.savefig(res)	
#@jit	
def saveRes():
...
def main():
	...
	global nb_type
	init()   #1.8, 0.3, 2, 2
	global buildup_or_not, T, merit_fig
	T=0
	while T <= 100:
		T+=1	
		...
		Cons=((X!=1)*(X!=9)*(X!=3)*(X!=5)*(X!=0)*(X!=-1)).astype(int)
		...
		if T==100:
			...
			np.place(X86,(X86==0),-1)
			np.place(X16_obs,(X16_obs==0),-1)

			np.place(X86,(X86==9),0)
			np.place(X16_obs,(X16_obs==9),0)

			np.place(X86,(X86!=1)*(X86!=-1),0)
			np.place(X16_obs,(X16_obs!=1)*(X16_obs!=-1),0)	

			np.place(X16_obs,(X16_obs-X86)==1,9)     #0 to 1: new urban area, observed

			#X16_sim=X
			np.place(X,(X!=1)*(X!=9)*(X!=-1),0)     #9 is new urban area
			...
			np.place(X,(X16_obs==9)*(X==0),2)
			np.place(X,(X16_obs==9)*(X==9),3)
			np.place(X,(X16_obs==0)*(X==9),4)
			np.place(X,(X==-1),5)
