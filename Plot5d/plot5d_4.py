...
def visualize3DData (X,scale,cmap):
	...
	#X0:a	X1:b	X2:r	X3:nb	X4:Figure_of_merit
	im= ax.scatter(X[:, 0], X[:, 1], X[:, 3], c= X[:, 4], s= X[:, 2], cmap=cmap, alpha=0.3, picker = True)
	...
	#loc=np.arange(3)+0.5
	#cbar.set_ticks(loc)
	#cbar.set_ticklabels(['Moore', 'vonNeumann', 'vonNeumannCircle'])	 #0,1,2	
	cbar.ax.set_ylabel('merit')
